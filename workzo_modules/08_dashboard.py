    ...
    if page_key == "dashboard":
        ...
        ">
            ...
            <div style="font-size:13px; color:#cbd5e1;">
                WorkZo AI is in beta. Results may not always be accurate. 
                Do not rely solely on AI-generated outputs for job applications.
                Always review and verify before using.
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        feedback = st.text_area(
            ...
            placeholder="Your feedback helps improve WorkZo...",
            key="workzo_feedback_input"
        )

        if st.button("Submit Feedback", key="workzo_feedback_submit"):
            try:
                ...
                st.caption("💡 Example: 'Job Assist is confusing' or 'Resume score not clear'")
        ...
        st.markdown(f"""
        <style>
        /* WorkZo v65: keep sidebar/toolbox buttons in original dark style, not red */
        ...
        with st.container(key="wz59_mobile_toolbox_native"):
            ...
            with c1:
                if st.button("🏠 Dashboard", key="wz60_mobile_dashboard", use_container_width=True):
                    _wz59_safe_mobile_go("dashboard")
                if st.button("🎯 Job Assist", key="wz60_mobile_jobs", use_container_width=True):
                    _wz59_safe_mobile_go("job_assist")
                if st.button("🤖 Work-O-Bot", key="wz60_mobile_bot", use_container_width=True):
                    _wz59_safe_mobile_go("workobot")
            with c2:
                if st.button("📄 CV", key="wz60_mobile_cv", use_container_width=True):
                    _wz59_safe_mobile_go("cv_documents")
                if st.button("Real Interview", key="wz60_mobile_real_interview", use_container_width=True):
                    _wz59_safe_mobile_go("real_interview")
                if st.button("📊 Founder", key="wz60_mobile_founder", use_container_width=True):
                    _wz59_safe_mobile_go("founder_dashboard")
    except Exception:
        pass
...
try:
    _wz60_previous_show_dashboard = show_dashboard
