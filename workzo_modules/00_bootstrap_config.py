...
def get_or_create_anonymous_user_id() -> str:
    ...
    This is not a real-person count. It is one stable browser id.
    The id is mirrored into the URL as wz_uid so refreshes and
    reruns do not create a fresh id and inflate Founder Analytics.
    """
    url_uid = _query_param_value("wz_uid", "")
    session_uid = st.session_state.get("anonymous_user_id", "")

    if _is_valid_uuidish(session_uid):
        uid = str(session_uid)
    elif _is_valid_uuidish(url_uid):
        uid = str(url_uid)
    else:
        uid = str(uuid.uuid4())

    try:
        if _query_param_value("wz_uid", "") != uid:
...
def render_founder_dashboard():
    ...
    browser_ids = {_uid(r) for r in rows if _uid(r)}
    unique_browser_ids = len(browser_ids)
    unique_sessions = len({_sid(r) for r in rows if _sid(r)})

    # Honest beta estimate: ignore one-off, very short ids that are usually
    # generated by refreshes, local testing, or Streamlit reruns.
    uid_event_counts = {}
    uid_max_duration = {}
    for _r in rows:
        _u = _uid(_r)
        if not _u:
            continue
        uid_event_counts[_u] = uid_event_counts.get(_u, 0) + 1
        uid_max_duration[_u] = max(uid_max_duration.get(_u, 0), _safe_int(_r.get("session_duration_seconds")))
    likely_tester_ids = {
        _u for _u in browser_ids
        if uid_event_counts.get(_u, 0) >= 2 or uid_max_duration.get(_u, 0) >= 30
    }
    short_or_duplicate_ids = max(0, unique_browser_ids - len(likely_tester_ids))
    ...
    return_rate = round((returning_users / len(likely_tester_ids)) * 100, 1) if likely_tester_ids else 0
    ...
    c5.metric("Returning browsers", returning_users, f"{return_rate}%")
    c6.metric("Browser IDs", unique_browser_ids)
    c7.metric("Activation rate", f"{activation_rate}%")
    c8.metric("Prepared jobs", len(prepared_sessions))

    st.caption(f"Honest mode: Likely testers excludes {short_or_duplicate_ids} one-off/very-short browser IDs. Browser IDs are not real people; the stable wz_uid URL value prevents refreshes from creating new IDs going forward.")

    st.markdown("#### Retention details")
    r1, r2, r3 = st.columns(3)
    r1.metric("Repeat browser IDs", returning_users)
    r2.metric("Browsers with 2+ sessions", sum(1 for s in user_sessions.values() if len(s) > 1))
    r3.metric("Browsers active 2+ days", sum(1 for d in user_dates.values() if len(d) > 1))
