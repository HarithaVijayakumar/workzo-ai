...
def _workzo_join_list_for_edit(items: Any) -> str:
    ...
    for item in _as_list(items):
        if isinstance(item, dict):
            name = item.get("name") or item.get("title") or item.get("degree") or item.get("company") or ""
            details = item.get("details") or item.get("description") or ""
            bullets = "\n".join(str(b) for b in _as_list(item.get("bullets")))
            text = "\n".join(x for x in [name, details, bullets] if str(x).strip())
            if text.strip():
    ...
    for item in _as_list(raw_projects):
        ...
        if isinstance(item, dict):
            normalized_projects.append({
                ...
                "bullets": [str(b).strip(" -•*") for b in _as_list(item.get("bullets") or item.get("details") or item.get("description")) if str(b or "").strip()],
            })
        elif str(item or "").strip():
...
def workzo_editable_cv_editor(prefix: str, cv_text: str = "", structured_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    updated = {
        ...
        "projects": [{"name": line, "bullets": []} for line in _workzo_parse_simple_lines(st.session_state.get(keys["projects"], ""))],
    ...
    for pr in _as_list(data.get("projects")):
        ...
        if isinstance(pr, dict):
            fixed["projects"].append({
                ...
                "bullets": [str(b).strip(" -•*") for b in _as_list(pr.get("bullets") or pr.get("details") or pr.get("description")) if str(b or "").strip()],
            })
