...
{{
  ...
  "best_fit_countries": ["Country 1 - short reason", "Country 2 - short reason", "Country 3 - short reason"],
...
- For best_fit_countries, suggest 3 countries where this CV/profile is likely to be competitive.
...
def show_workobot():
    ...
    with qp1:
        if st.button("B1/B2 mock questions", use_container_width=True):
            quick_prompt = "Give me a mock test practice set with example questions and answers."
    with qp2:
        if st.button("Career communication", use_container_width=True):
            quick_prompt = "Help me improve my career communication with 3 examples."
    with qp3:
        if st.button("Skill gap help", use_container_width=True):
            quick_prompt = "Based on my CV, what skill gap should I fix first?"
    with qp4:
        if st.button("German for interviews", use_container_width=True):
            quick_prompt = "Give me German interview questions and sample answers for my profile."
...
"""
        ...
        if st.button(txt("save_resume"), key="btn_save_updated_resume"):
...
"""
    ...
    # =====================================================
        ...
        search_country = st.selectbox("Country for job search", target_country_options, index=country_index)
        ...
        target_titles = st.text_input("Optional target job titles", placeholder="Example: Data Analyst, Reporting Analyst, IT Support")
        ...
        if st.button(txt("generate"), key="btn_find_jobs_v36"):
            if not cv_for_jobs.strip():
                ...
                with st.spinner("Searching actual jobs and building a stronger plan..."):
                    ...
                    for role in roles_from_input[:5]:
                        st.markdown(f"**{role}**")
                        for label, url in get_job_board_links(search_country, final_location, role):
                            st.markdown(f"- [{label}]({url})")
    
                    st.caption("Germany can use Bundesagentur für Arbeit and Arbeitnow. Supported Adzuna countries can also show live jobs when ADZUNA_APP_ID and ADZUNA_APP_KEY are configured. If live jobs are unavailable, WorkZo still shows a structured application plan and country-specific job-board links.")
        # =====================================================
    # MOCK INTERVIEW
    # =====================================================
        ...
        interview_language = st.selectbox(
            ...
            [lang for lang in ["English", "German", "Dutch"] if lang in language_options] or language_options[:3]
        )
        interview_background = st.text_area(txt("background"), value=st.session_state.cv_text, height=240)

        if st.button(txt("generate"), key="btn_mock_interview_v30"):
...
"""
    ...
    # =====================================================
        ...
        if st.button(txt("analyze"), key="btn_skill_gap_v30"):
...
"""
        ...
        if st.button("Discuss My Gaps", key="btn_gap_followup_v30"):
...
"""
    ...
    # =====================================================
        ...
        if st.button(txt("generate"), key="btn_roadmap_v30"):
