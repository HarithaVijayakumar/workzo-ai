...
def run_ai_prompt(prompt: str, system_addition: str = "", force_english: bool = False) -> str:
    if not can_make_request():
        return "ERROR: Usage limit reached. Please try again later."

    register_request()

    answer_lang = "English" if force_english else (st.session_state.response_language or "English")
...
with top3:
    st.session_state.response_language = st.selectbox(
        ...
        ["English", "German", "Dutch"],
        index=["English", "German", "Dutch"].index(st.session_state.response_language)
        if st.session_state.response_language in ["English", "German", "Dutch"] else 0
    )
...
def show_dashboard():
    with st.sidebar:
        ...
        st.info(f"{txt('country')}: {st.session_state.country}")
        st.info(f"{txt('resume_loaded')}: {txt('yes') if st.session_state.cv_text.strip() else txt('no')}")
    ...
    st.success(txt("onboarding_complete"))
