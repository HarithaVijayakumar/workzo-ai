...
def maybe_render_founder_access():
    ...
    with st.sidebar.expander("Founder access", expanded=False):
        entered_pin = st.text_input("Founder PIN", type="password", key="founder_pin_input")
    ...
    if st.session_state.get("founder_unlocked"):
        with st.sidebar:
            if st.button("Open Founder Dashboard", key="open_founder_dashboard"):
                st.session_state.page = "Founder Dashboard"
...
SUPPORTED_STRICT_LANGUAGES = {"English", "German", "Dutch"}

def normalize_answer_language(language: str) -> str:
    language = (language or "English").strip()
    if language in SUPPORTED_STRICT_LANGUAGES:
        return language
    return "English"
...
with header_col2:
    ...
    <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
        ...
        <span class="beta-badge">BETA V5.4</span>
    </div>
...
"""
    ...
    st.divider()
    st.caption("WORKZO AI V5.4 • Advanced multilingual AI • Stricter ATS scoring • Improved document tools")
