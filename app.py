...
The reading order may be wrong. Your task is to classify facts by meaning, NOT by raw order.
...
Critical rules:
- Do NOT preserve the raw PDF order.
- Classify every useful line into the correct resume section.
- If Education appears visually near the top, still place it under education.
- If Skills/Expertise appear before the name, still place them under skills/tools.
- If Contact appears inside profile text, move it to contact.
- If dates appear near another section, attach dates only to the most likely employer/degree/project based on nearby words.
...
- Remove generic soft skills unless they are clearly important for the target role.
- Put uncertain lines into details_to_confirm.
...
def render_structured_cv_review(target_country: str, user_status: str) -> None:
    ...
    with st.form("structured_cv_review_form"):
        ...
        back = c_back.form_submit_button("⬅️ Re-upload / edit setup", use_container_width=True)
    ...
    if approve:
        approved = {
            ...
            "projects": projects,
            "education": [x.strip(" -•") for x in education.splitlines() if x.strip(" -•")],
...
def get_clean_cv_source_for_tools() -> str:
    """Return the reviewed structured CV for tools, falling back only when needed."""
    for key in ["clean_structured_cv_text", "structured_cv_profile", "cv_text"]:
        value = st.session_state.get(key, "")
        if isinstance(value, str) and len(value.strip()) > 80:
            return strip_markdown_for_resume(value)
    return strip_markdown_for_resume(st.session_state.get("cv_text", ""))
...
def show_onboarding():
    ...
    if st.session_state.get("cv_review_required"):
        ...
        render_structured_cv_review(st.session_state.migration_country, user_status)
        return
    ...
    with c_upload:
        active = " ✅" if st.session_state.get("cv_mode") == "Upload CV" else ""
        if st.button(f"📄 {txt('upload_cv_title')}{active}\n\n{txt('upload_cv_desc')}", key="choose_upload_cv", use_container_width=True):
            st.session_state.cv_mode = "Upload CV"
            st.rerun()
    with c_create:
        active = " ✅" if st.session_state.get("cv_mode") == "Create CV" else ""
        if st.button(f"✍️ {txt('create_cv_title')}{active}\n\n{txt('create_cv_desc')}", key="choose_create_cv", use_container_width=True):
            st.session_state.cv_mode = "Create CV"
            st.rerun()
    with c_linkedin:
        active = " ✅" if st.session_state.get("cv_mode") == "LinkedIn" else ""
        if st.button(f"🔗 {txt('linkedin_cv_title')}{active}\n\n{txt('linkedin_cv_desc')}", key="choose_linkedin_cv", use_container_width=True):
...
""".strip()
        ...
        if submitted:
            ...
            if cv_mode == "Upload CV":
                ...
                st.session_state.pending_structured_cv_json = structured_data
                st.session_state.cv_review_required = True
                track_event("cv_structured_review_started", "Onboarding", {"cv_mode": cv_mode, "target_country": st.session_state.get("migration_country", country)})
                request_scroll_to_top()
                st.rerun()
...
def parse_cv_sections_for_template(cv_text: str) -> Dict[str, str]:
    ...
    def is_location(s):
        lower = s.lower()
        return any(x in lower for x in ["würzburg", "wuerzburg", "berlin", "munich", "germany", "chennai", "india", "straße", "strasse", "weg "]) or bool(re.search(r"\b\d{5}\b", s))
    def looks_like_name(s):
        cleaned = re.sub(r"[^A-Za-zÄÖÜäöüß ]", "", s).strip()
        if not cleaned:
            return False
        words = cleaned.split()
    ...
    heading_map = {
        "summary": ["summary", "profile summary", "profile", "professional summary", "professional profile", "career objective", "objective"],
        "skills": ["skills", "core skills", "technical skills", "expertise", "kenntnisse"],
        "experience": ["experience", "work experience", "professional experience", "employment", "berufserfahrung"],
        "projects": ["projects", "project", "academic projects", "projekte"],
        "education": ["education", "ausbildung", "academic", "studies"],
...
- Do not move phone number, email, LinkedIn, or location into the Professional Summary. Keep them only in the header.
- Do not put contact details under PROFILE.
- Do not output stray words such as PROFESSIONAL, CORE, CONTACT, ADDITIONAL, or placeholder text.
- Do not merge unrelated sections together.
- Build the CV from facts/keywords into clean sections; do not copy the old CV layout.
        ...
        if st.session_state.get("improved_cv_text_v92"):
            ...
            with dl_col4:
                if rendercv_pdf:
                    ...
                    st.download_button(label=ui_label("Download RenderCV YAML"), data=rendercv_yaml, file_name=f"{safe_file_base}_rendercv.yaml", mime="text/yaml", key="download_improved_cv_rendercv_yaml_v1", help=ui_label("Experimental: install RenderCV and test this YAML to generate a LaTeX-style CV."))
        ...
        if st.session_state.get("generated_country_cv_text"):
            ...
            with dl_col4:
                if rendercv_pdf:
                    ...
                    st.download_button(label="Download RenderCV YAML", data=rendercv_yaml, file_name=f"{safe_file_base}_rendercv.yaml", mime="text/yaml", key="download_country_cv_rendercv_yaml_v1", help="Experimental: install RenderCV and test this YAML to generate a LaTeX-style CV.")
...
def show_dashboard():
    ...
    if page_key == "dashboard":
        ...
        progress_items = [
            ("1", ui_label("CV added"), cv_ready, "dashboard", ui_label("Edit CV")),
        ...
        for col, (num, title, done, target, button_text) in zip(progress_cols, progress_items):
            with col:
                ...
                """, unsafe_allow_html=True)
                if st.button(button_text + " →", key=f"dashboard_progress_nav_{num}_{target}", use_container_width=True):
                    if target == "dashboard":
                        reset_onboarding()
                        request_scroll_to_top()
                        st.rerun()
