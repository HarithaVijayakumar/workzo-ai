...
<style>
    ...
    /* WorkZo v9.26: mobile sidebar recovery + visible hamburger/collapse control */
    button[kind="header"],
...
# WorkZo v9.26 translated labels used in newer pages
EXTRA_UI_TEXT_V926 = {
    "English": {
        "sidebar_profile": "Profile", "sidebar_settings": "Settings", "sidebar_navigation": "Navigation", "sidebar_admin": "Admin",
        "country_label": "Country", "career_status_label": "Career status", "resume_label": "Resume", "target_market_label": "Target market",
        "resume_uploaded": "Resume uploaded", "no_resume_yet": "No resume yet",
        "prepare_job_tab": "Prepare for this Job", "ai_job_application_assistant": "AI Job Application Assistant",
        "prepare_job_caption": "Paste a job description. WorkZo will prepare your CV, cover letter focus, interview plan, skill roadmap, and country-specific application guidance.",
        "target_company": "Target company", "target_company_placeholder": "Example: Siemens, SAP, HubSpot",
        "target_job_title": "Target job title", "target_job_title_placeholder": "Example: Technical Support Engineer",
        "prepare_full_application": "Prepare my full application", "warn_job_desc_first": "Please paste the job description first.",
        "warn_cv_first": "Please upload or create your CV first.", "usage_limit_reached": "Usage limit reached. Please try again later.",
        "preparing_application": "Preparing your job application package...", "choose_pdf_txt_docx": "Choose a PDF, DOCX, or TXT file above to continue.",
        "resume_option_caption": "Click one option below. The card itself opens the input area — there is no second button.",
        "linkedin_format_info": "After importing LinkedIn, WorkZo turns your link and notes into a CV draft. You can find it in CV & Documents → Country-Specific Resume Builder, and it is also saved as your resume text on the dashboard.",
        "enter_few_details": "Please enter at least a few details so WorkZo can build your CV.", "creating_cv_draft": "Creating your CV draft with AI...",
        "cv_upload_success": "CV uploaded successfully. You can continue to the dashboard."
    },
    "German": {
        "sidebar_profile": "Profil", "sidebar_settings": "Einstellungen", "sidebar_navigation": "Navigation", "sidebar_admin": "Admin",
        "country_label": "Land", "career_status_label": "Karrierestatus", "resume_label": "Lebenslauf", "target_market_label": "Zielmarkt",
        "resume_uploaded": "Lebenslauf hochgeladen", "no_resume_yet": "Noch kein Lebenslauf",
        "prepare_job_tab": "Für diese Stelle vorbereiten", "ai_job_application_assistant": "KI-Bewerbungsassistent",
        "prepare_job_caption": "Füge eine Stellenbeschreibung ein. WorkZo bereitet Lebenslauf-Fokus, Anschreiben, Interviewplan, Skill-Roadmap und länderspezifische Bewerbungshinweise vor.",
        "target_company": "Zielunternehmen", "target_company_placeholder": "Beispiel: Siemens, SAP, HubSpot",
        "target_job_title": "Zielposition", "target_job_title_placeholder": "Beispiel: Technical Support Engineer",
        "prepare_full_application": "Meine vollständige Bewerbung vorbereiten", "warn_job_desc_first": "Bitte füge zuerst die Stellenbeschreibung ein.",
        "warn_cv_first": "Bitte lade zuerst deinen Lebenslauf hoch oder erstelle ihn.", "usage_limit_reached": "Nutzungslimit erreicht. Bitte versuche es später erneut.",
        "preparing_application": "Dein Bewerbungspaket wird vorbereitet...", "choose_pdf_txt_docx": "Wähle oben eine PDF-, DOCX- oder TXT-Datei aus, um fortzufahren.",
        "resume_option_caption": "Klicke unten direkt auf eine Option. Die Karte selbst öffnet den Eingabebereich — es gibt keinen zweiten Button.",
        "linkedin_format_info": "Nach dem LinkedIn-Import verwandelt WorkZo deinen Link und deine Notizen in einen CV-Entwurf. Du findest ihn unter Lebenslauf & Dokumente → Länderspezifischer Resume Builder; außerdem wird er als Lebenslauftext im Dashboard gespeichert.",
        "enter_few_details": "Bitte gib mindestens ein paar Details ein, damit WorkZo deinen Lebenslauf erstellen kann.", "creating_cv_draft": "Dein Lebenslauf-Entwurf wird mit KI erstellt...",
        "cv_upload_success": "Lebenslauf erfolgreich hochgeladen. Du kannst zum Dashboard fortfahren."
    }
}
for _lang, _items in EXTRA_UI_TEXT_V926.items():
    UI_TEXT.setdefault(_lang, {}).update(_items)
...
def render_live_jobs(jobs: List[Dict], country_name: str = "", max_visible: int = 30, roles: Optional[List[str]] = None, cv_text: str = ""):
    ...
    filtered_jobs = [job for job in jobs if job_location_relevant(job, country_name)]
    if country_name:
        jobs = filtered_jobs
    ...
    possible_jobs = [job for job in ranked_jobs if 50 <= job.get("_match_score", 0) < 70]
    hidden_jobs = [job for job in ranked_jobs if job.get("_match_score", 0) < 50]
    ...
    st.markdown(f"### {title}")
    st.caption("Only jobs with at least 50% profile fit are shown. Weak matches are hidden to keep the results useful.")
    st.caption(f"Showing {len(visible_jobs)} relevant openings. Hidden weak matches: {len(hidden_jobs)}.")

    if not visible_jobs:
        st.info("No strong or possible matches found yet. Try a broader role title, a wider location, or add more skills to your CV profile.")
        return

    top_jobs = strong_jobs[:5] if strong_jobs else visible_jobs[:5]
    remaining_jobs = [job for job in visible_jobs if job not in top_jobs]
...
<a href="{html.escape(url, quote=True)}" target="_blank" style="text-decoration:none; color:inherit;">
  <div class="card" style="cursor:pointer; transition:0.15s; border-color:rgba(96,165,250,0.32);">
    <div style="display:flex; justify-content:space-between; gap:12px; align-items:flex-start;">
      ...
      <div class="beta-badge">{match_score}% match</div>
    </div>
...
"""
    ...
    if remaining_jobs:
        with st.expander(f"Show details — {len(remaining_jobs)} more jobs", expanded=False):
...
def render_job_board_search_cards(country_name: str, location: str, roles: List[str], user_status: str = ""):
    st.markdown("### Find More Jobs On")
    st.caption("Open current searches on major job platforms for each suggested role.")
    for role in roles[:3]:
    ...
    if isinstance(titles, list) and titles:
        ...
        for item in titles[:3]:
            if isinstance(item, dict):
                role_names.append(str(item.get("title", "Role")))
            else:
                role_names.append(str(item))
        if role_names:
            chips = "".join([f"<span class='pill'>{i}. {html.escape(role)}</span>" for i, role in enumerate(role_names[:3], start=1)])
            st.markdown(f"""
<div class="card" style="padding:12px 14px; border-color:rgba(20,184,166,0.24); background:rgba(15,23,42,0.42);">
  <div class="small-muted" style="margin-bottom:8px;"><b>Based on your CV, these roles fit best:</b></div>
  <div>{chips}</div>
  <div class="small-muted" style="margin-top:8px;">You are closest to these roles.</div>
</div>
""", unsafe_allow_html=True)
        ...
        for item in titles[:3]:
            if isinstance(item, dict):
                ...
                if len(fit_reason) > 180:
                    fit_reason = fit_reason[:180].rsplit(" ", 1)[0] + "..."
                card_html = f"""
<div class="card" style="border-color:rgba(20,184,166,0.22);">
  <div class="section-title">{html.escape(title)}</div>
  <div class="small-muted">Experience level: {html.escape(seniority)} • Language: {html.escape(english_realistic)}</div>
  <div style="margin-top:10px;"><strong>Why this role fits you:</strong></div>
  <div style="margin-top:6px; color:#cbd5e1;">{html.escape(fit_reason)}</div>
</div>
"""
...
def clean_cv_text(raw_text: str) -> str:
    ...
    text = text.replace("C ++", "C++").replace("C #", "C#").replace("Java Script", "JavaScript")
...
def extract_pdf_text(uploaded_file) -> str:
    """Extract CV text from PDFs, including code-heavy CVs where normal text extraction can be sparse."""
    text_parts = []
    try:
...
def extract_docx_text(uploaded_file) -> str:
    """Extract text from DOCX CVs when users upload Word resumes."""
    if Document is None:
        raise RuntimeError("python-docx is not installed. Add python-docx to requirements.txt to support DOCX uploads.")
    try:
        uploaded_file.seek(0)
    except Exception:
        pass
...
def show_onboarding():
    ...
    st.caption(txt("resume_option_caption"))
    c_upload, c_create, c_linkedin = st.columns(3)
    with c_upload:
        upload_label = f"📄  {txt('upload_cv_title')}\n\n{txt('upload_cv_desc')}"
        if st.button(upload_label, key="choose_upload_cv", use_container_width=True):
            st.session_state.cv_mode = "Upload CV"
            st.rerun()
    with c_create:
        create_label = f"✍️  {txt('create_cv_title')}\n\n{txt('create_cv_desc')}"
        if st.button(create_label, key="choose_create_cv", use_container_width=True):
            st.session_state.cv_mode = "Create CV"
            st.rerun()
    with c_linkedin:
        linkedin_label = f"🔗  {txt('linkedin_cv_title')}\n\n{txt('linkedin_cv_desc')}"
        if st.button(linkedin_label, key="choose_linkedin_cv", use_container_width=True):
    ...
    if cv_mode == "Upload CV":
        ...
        if uploaded_direct is not None:
            ...
            st.success(txt("cv_upload_success"))
        ...
        if submitted:
            ...
            if cv_mode == "Upload CV":
                ...
                if cv_mode == "Create CV" and not full_name.strip():
                    ...
                    return
                if not cv_text_input.strip():
                    st.warning(txt("enter_few_details"))
                    return
...
def show_dashboard():
    ...
    with st.sidebar:
        ...
        <div class='workzo-sidebar-brand-wrap'>
            ...
            <div class='workzo-sidebar-version'>Beta V9.26 · mobile + translation fixes</div>
        </div>
        """, unsafe_allow_html=True)
        st.button("🏠 " + txt("dashboard"), key="sidebar_dashboard_home_button", use_container_width=True, on_click=queue_navigation, args=("dashboard",))

        st.markdown(f"<div class='workzo-sidebar-section-label'>{html.escape(txt('sidebar_profile'))}</div>", unsafe_allow_html=True)
        country_sidebar = html.escape(str(st.session_state.get('country', txt('not_specified')) or txt('not_specified')))
        st.markdown(f"<div class='workzo-sidebar-chip'><span class='workzo-profile-chip-label'>{html.escape(txt('country_label'))}</span><span class='workzo-profile-chip-value'>🌍 {country_sidebar}</span></div>", unsafe_allow_html=True)
        ...
        if user_status_sidebar.strip():
            st.markdown(f"<div class='workzo-sidebar-chip'><span class='workzo-profile-chip-label'>{html.escape(txt('career_status_label'))}</span><span class='workzo-profile-chip-value'>💼 {html.escape(user_status_sidebar)}</span></div>", unsafe_allow_html=True)
        resume_status = txt("resume_uploaded") if str(st.session_state.get("cv_text", "")).strip() else txt("no_resume_yet")
        st.markdown(f"<div class='workzo-sidebar-chip'><span class='workzo-profile-chip-label'>{html.escape(txt('resume_label'))}</span><span class='workzo-profile-chip-value'>📄 {html.escape(resume_status)}</span></div>", unsafe_allow_html=True)
        if st.session_state.get("migration_country") and st.session_state.get("migration_country") != st.session_state.country:
            st.markdown(f"<div class='workzo-sidebar-chip'><span class='workzo-profile-chip-label'>{html.escape(txt('target_market_label'))}</span><span class='workzo-profile-chip-value'>🎯 {html.escape(str(st.session_state.migration_country))}</span></div>", unsafe_allow_html=True)

        st.markdown(f"<div class='workzo-sidebar-section-label'>{html.escape(txt('sidebar_settings'))}</div>", unsafe_allow_html=True)
        language_list = language_options if language_options else ["English", "German", "Dutch"]
        ...
        st.markdown(f"<div class='workzo-sidebar-section-label'>{html.escape(txt('sidebar_navigation'))}</div>", unsafe_allow_html=True)
        ...
        st.markdown(f"<div class='workzo-sidebar-section-label'>{html.escape(txt('sidebar_admin'))}</div>", unsafe_allow_html=True)
        founder_pin = os.getenv("FOUNDER_PIN") or get_streamlit_secret("FOUNDER_PIN")
    ...
    if page_key == "dashboard":
        ...
        <div class="workzo-dashboard-hero-compact">
            <div class="workzo-hero-kicker">{txt('career_command_center')}</div>
            <div class="workzo-hero-title">{txt('career_move_organized')}</div>
        ...
        s1, s2 = st.columns(2)
        with s1:
            render_metric_card(txt("resume_score"), str(st.session_state.cv_score_value or "—"), f"{score_band(int(st.session_state.cv_score_value or 0))} · {txt("metric_resume_quality")}" if st.session_state.cv_score_value else txt("metric_resume_quality"))
        with s2:
            render_metric_card(txt("ats_score"), str(st.session_state.ats_score_value or "—"), f"{score_band(int(st.session_state.ats_score_value or 0))} · {txt("metric_ats_friendly")}" if st.session_state.ats_score_value else txt("metric_ats_friendly"))
        ...
        with improve_col:
            ...
            st.markdown(country_aware_resume_improvements(st.session_state.resume_improvements) if st.session_state.resume_improvements else f"<span class='small-muted'>{txt('not_analyzed')}</span>", unsafe_allow_html=True)
...
"""
            ...
            st.markdown(f"### {txt('ai_job_application_assistant')}")
            st.caption(txt("prepare_job_caption"))
            ...
            if st.button(txt("prepare_full_application"), key="btn_prepare_full_application_v928", use_container_width=True):
                track_button_click(txt("prepare_full_application"), txt("ai_job_application_assistant"))
                if not job_desc_prepare.strip():
                    st.warning(txt("warn_job_desc_first"))
                elif not cv_text_for_prepare.strip():
                    st.warning(txt("warn_cv_first"))
                elif not can_make_request():
                    st.warning(txt("usage_limit_reached"))
                else:
                    with st.spinner(txt("preparing_application")):
                        register_request()
...
"""
            ...
            if latest_prep:
                ...
                if readiness_score is not None:
                    ...
                    with c3:
                        st.metric(txt("target_company"), target_company or txt("not_specified"))
...
url_page = read_url_page(st.session_state.get("page", "dashboard"))
valid_pages = {"dashboard", "job_assist", "cv_documents", "workobot", "founder_dashboard", "onboarding"}
if url_page in valid_pages and st.session_state.get("onboarding_complete"):
    st.session_state.page = url_page
    st.session_state.nav_page = "dashboard" if url_page == "onboarding" else url_page
