...
with header_col2:
    ...
    <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
        ...
        <span class="beta-badge">BETA V8.0</span>
    </div>
...
def show_onboarding():
    ...
    preferred_language = st.selectbox(
        ...
        help="This language is used for the app, AI replies, and generated documents."
    )
    ...
    status_display = st.selectbox(
        ...
        help="Optional. Select this only if it matches your current situation."
    )
    status_map = {
    ...
    user_status = status_map.get(status_display, status_display).strip()
    ...
    st.caption("Choose one option to add your resume. You can upload an existing CV or create one with the guided builder.")
    ...
    with c_upload:
        ...
        <div class="choice-card">
            <div class="choice-card-title">📄 Upload CV</div>
            <div class="choice-card-copy">Best if you already have a PDF or text CV.</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with c_create:
        st.markdown("""
        <div class="choice-card">
            <div class="choice-card-title">✍️ Create CV</div>
            <div class="choice-card-copy">Best if you want WorkZo to build a CV from your details.</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not cv_mode:
        st.info("Please choose Upload CV or Create CV to continue.")
        return
    ...
    with st.form("onboarding_form_v48"):
        ...
        if cv_mode == "Upload CV":
            ...
            if uploaded_file is not None:
                ...
                st.info("Please upload a PDF or TXT CV using the Upload CV option above.")
            ...
            with c1:
                ...
                location = st.text_input("Location", placeholder="Example: Berlin, Germany")
            with c2:
                languages = st.text_area("Languages", placeholder="Example: English B2, German A2, Spanish native")
                certifications = st.text_area("Certifications / Courses", placeholder="Example: Google Data Analytics Certificate, Excel Advanced, AWS Cloud Practitioner basics")
            ...
            projects = st.text_area("Projects", placeholder="Example: Built a sales dashboard in Tableau using sample sales data to track monthly revenue and customer trends.")
...
def go_to_nav(page_key: str):
    # Keep the sidebar radio widget and internal page state in sync.
    # This makes dashboard buttons actually navigate to the selected page.
...
def show_dashboard():
    ...
    if page_key == "dashboard":
        ...
        with h3:
            ...
            if st.button(txt("go_workobot"), use_container_width=True, key="dash_go_workobot"):
                go_to_nav("workobot")
            ...
            if st.button(txt("generate"), key="btn_find_jobs_v42"):
                ...
                if not cv_for_jobs.strip():
                    ...
                    with st.spinner("Searching worldwide live jobs..."):
                        ...
                        st.markdown("### Best live search links")
                        st.caption("These open current results directly on major job boards. Use them when API results are limited or too narrow.")
...
"""
        ...
        st.subheader(txt("workobot"))
        st.caption(txt("workobot_desc_short"))
        wb_tab1, wb_tab2, wb_tab3 = st.tabs(["🎤 Mock Interview", "📊 Career Insights", "💬 Career Chat"])
        with wb_tab1:
        ...
        with wb_tab2:
            ...
            show_workobot()
        with wb_tab3:
            st.session_state.workobot_mode = "Career Communication"
            show_workobot()
