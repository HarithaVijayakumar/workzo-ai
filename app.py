...
LANG_EXTRA = {
    "English": {
        ...
        "career_command_center": "{txt('career_command_center')}", "career_move_organized": "{txt('career_move_organized')}",
...
"""
    result = run_ai_prompt(prompt, force_language=st.session_state.get("response_language", "English"), json_mode=True)
...
- Follow the selected AI response language strictly. Do not mix languages.
""".strip()
...
def run_ai_prompt(
    ...
    if force_english:
        ...
        answer_lang = normalize_answer_language(st.session_state.get("response_language", "English"))
...
def show_dashboard():
    with st.sidebar:
        ...
        preferred = st.selectbox(
            ...
            help="This single language controls both AI responses and app labels when supported. Unsupported UI languages use English labels but AI answers in your selected language."
        )
        st.session_state.preferred_language = preferred
        st.session_state.ui_language = preferred if preferred in UI_TEXT else "English"
        st.session_state.response_language = preferred
        st.caption(f"{txt('status_label')}: {st.session_state.get('user_status', txt('not_specified'))}")
        ...
        if st.session_state.get("founder_unlocked"):
            action_labels.append(txt("founder_dashboard"))

        page = st.radio(txt("navigation"), action_labels, label_visibility="collapsed")
        track_feature_view(page)
    ...
    if page == txt("dashboard"):
        ...
        with m3:
            render_metric_card(txt("detected_role"), st.session_state.current_role_detected or txt("not_analyzed"), txt("metric_detected_resume"))
        with m4:
            ...
            render_metric_card(txt("target_roles"), str(roles_count or "—"), txt("metric_role_cluster"))
        ...
        with f1:
            render_feature_tile("🧭", txt("job_assist"), txt("job_assist_desc"))
        with f2:
            render_feature_tile("📝", txt("document_tools"), txt("document_tools_desc"))
        with f3:
            render_feature_tile("🤖", txt("workobot"), txt("workobot_desc"))
        ...
        st.markdown(f"### {txt('standout_next_best_step')}")
        st.caption(txt("next_best_caption"))
        action_col1, action_col2 = st.columns([1, 1.2])
        with action_col1:
            if st.button(txt("generate_next_best_step"), use_container_width=True, key="btn_next_best_step"):
                with st.spinner(txt("building_action_center")):
            ...
            if st.button(txt("refresh_resume"), use_container_width=True):
                with st.spinner(txt("refreshing")):
                    analyze_resume_dashboard_stable(st.session_state.cv_text, force_refresh=True)
                st.success(txt("dashboard_updated"))
                st.rerun()
        with action_col2:
...
"""
    ...
    elif page == txt("founder_dashboard"):
        if st.session_state.get("founder_unlocked"):
        ...
        show_workobot()
    render_feedback_collector(page if isinstance(page, str) else "General")

    st.divider()
    st.caption("WORKZO AI V6.0 • Cleaner UX • Clickable jobs • Founder analytics • Smarter country insights")
